        ...
        public ActionResult Index(System.DateTime? searchTime, int? searchId, string nameCus, int? DropdownStatus)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (nameCus != null)
                {
                    ...
                    listCTDH = listCTDH.Where(m => m.DonHang.Customer.HoTen.ToLower().Contains(nameCus.ToLower())).ToList();
                }
            ...
            using (var stream = upload.InputStream)
            {
                ...
                while (reader.Read())
                {
                    if (count > 0)
                    {
                        ...
                        try
                        {
                            statusOrder = Int32.Parse(reader.GetString(7));
                        }
                        catch
                        {
                        ...
                        if (statusOrder == 4)
                        {
                            ...
                            MailHelper.SendMailOrderReceived(db.Merchants.Find(ctdonhang.MerchantID).Email, "KnowledgeStore thông báo tình trạng đơn hàng", ctdonhang.ChiTietDonHangID.ToString(), db.Merchants.Find(ctdonhang.MerchantID).TenCuaHang, "giao thành công!!!", System.DateTime.Now.ToString("dd/MM/yyyy"));
                            ...
                            MailHelper.SendMailOrderReceived(db.Merchants.Find(ctdonhang.MerchantID).Email, "KnowledgeStore thông báo tình trạng đơn hàng", ctdonhang.ChiTietDonHangID.ToString(), db.Merchants.Find(ctdonhang.MerchantID).TenCuaHang, "đã giao thất bại, xu hoa hồng đc hoàn trả cho quí khách", System.DateTime.Now.ToString("dd/MM/yyyy"));
